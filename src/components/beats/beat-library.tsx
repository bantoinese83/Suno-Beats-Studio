  ...
  if (isLoading) {
    return (
      <div className="space-y-4 pt-12">
        <h2 className="text-[10px] font-bold uppercase tracking-widest text-muted">Architectural Archive</h2>
        <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
          {[1,2,3].map(i => (
             <div key={i} className="h-32 w-64 shrink-0 animate-pulse rounded-2xl bg-white/[0.03]" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4 pt-12 animate-in">
      <div className="flex items-center justify-between">
        <h2 className="text-[10px] font-bold uppercase tracking-widest text-muted">Architectural Archive</h2>
        <span className="text-[10px] text-muted-foreground/50">{history.length} Blueprints</span>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
        {history.map((beat) => (
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-mono text-muted-foreground/60">{beat.taskId.substring(0, 8)}</p>
              {beat.isFavorite && (
